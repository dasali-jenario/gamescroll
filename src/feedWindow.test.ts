...
describe('appendFeedWindow', () => {
  ...
  it('uses roadmap defaults that bound ~2–3 catalog batches', () => {
    ...
    const result = appendFeedWindow(items(FEED_WINDOW_MAX), items(batch, FEED_WINDOW_MAX), {
      activeIndex: 90,
    })
    expect(result.feed.length).toBe(FEED_WINDOW_MAX)
    expect(result.removedCount).toBe(batch)
    expect(result.activeIndex).toBe(90 - batch)
  })
